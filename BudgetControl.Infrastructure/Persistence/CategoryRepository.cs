...
public class CategoryRepository : ICategoryRepository<LedgerCategory>
{
    ...
    {
        var ledger = LedgerRepository.ledgers.SingleOrDefault(c => c.Id.Value.ToString() == ledgerId);
        return ledger?.Categories.SingleOrDefault(c => c.Id.Value.ToString() == id);
    }
    ...
    {
        var index = LedgerRepository.ledgers.FindIndex(c => c.Id.Value.ToString() == ledgerId);
    ...
    {
        var ledgerIndex = LedgerRepository.ledgers.FindIndex(c => c.Id.Value.ToString() == ledgerId);
        var categoryIndex = LedgerRepository.ledgers[ledgerIndex].Categories.FindIndex(c => c.Id.Value.ToString() == category.Id.Value.ToString());
    }
    ...
    public void Remove(string ledgerId, string id)
    {
        ...
        LedgerRepository.ledgers[index]
            .Categories.Remove(
                LedgerRepository.ledgers[index].Categories.Single(c => c.Id.Value.ToString() == id)
            );
    }
    ...
    public List<LedgerCategory> GetList(string ledgerId)
    {
        return LedgerRepository.ledgers
                    .SingleOrDefault(
                        c => c.Id.Value.ToString() == ledgerId
